...
{{-- <div class="slide-navigation main_menu d-none d-lg-block">
            ...
            <div class="col-xl-12">
                <div class="relative_contect d-flex">
                    ...
                    <ul class="wsus_menu_cat_item show_home toggle_menu">
                        ...
                        @foreach ($categories as $category)
                        <li><a class="{{count($category->subCategories) > 0 ? 'droap_arrow' : ''}}" href="{{route('products.index', ['category' => $category->slug])}}"><i class="{{$category->icon}}"></i> {{$category->name}} </a>
                            @if(count($category->subCategories) > 0)
...
<section id="mobile_menu">
    <span class="mobile_menu_close"><i class="fal fa-times" style="margin-top: 5px;" ></i></span>
    <ul class="mobile_menu_header_icon d-inline-flex">

        <li><a href="{{route('user.wishlist.index')}}"><i class="fal fa-heart" style="margin-top: 5px;"></i>
        ...
        @if (auth()->check())
        @if (auth()->user()->role === 'user')
        <li><a href="{{route('user.dashboard')}}"><i class="fal fa-user" style="margin-top: 5px;"></i></a></li>
        @elseif (auth()->user()->role === 'vendor')
    ...
    <div class="tab-content" id="pills-tabContent">
        <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
            <div class="mobile_menu_main_menu">
                <div class="accordion accordion-flush" id="accordionFlushExample">
                    <ul class="wsus_mobile_menu_category">
                        @foreach ($categories as $categoryItem)
                        <li>
                            <a href="{{route('products.index', ['category_slug' => $categoryItem->slug])}}" class="{{ count($categoryItem->subCategories) > 0 ? 'accordion-button' : '' }} collapsed" data-bs-toggle="collapse"
                                    ...
                                    <div class="accordion-body">
                                        <ul>
                                            @foreach ($categoryItem->subCategories as $subCategoryItem)
                                                <li><a href="{{ route('products.index', ['category_slug' => $categoryItem->slug, 'subcategory_slug' => $subCategoryItem->slug]) }}">
