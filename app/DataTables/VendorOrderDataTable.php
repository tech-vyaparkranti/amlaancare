    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                $showBtn = "<a href='".route('vendor.orders.show', $query->id)."' class='btn btn-primary me-2'><i class='far fa-eye'></i></a>";
                $showBtn .= "<a href='".route('vendor.orders.shipment', $query->id)."' class='btn btn-info'><i class='far fa-shipping-fast'></i></a>";
                return $showBtn;
            })
            ...
            ->addColumn('product_qty', function($query){
                $orderProducts = $query->orderProducts->where('vendor_id', Auth::user()->vendor->id);
                $totalQty = $orderProducts->sum('qty');  // Now sum the qty of products with the correct vendor_id
                return $totalQty;
            })
            ->addColumn('amount', function($query){
                $orderProducts = $query->orderProducts->where('vendor_id', Auth::user()->vendor->id);
