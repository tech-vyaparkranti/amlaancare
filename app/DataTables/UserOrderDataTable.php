    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                ...
                // Cancel button (only if order is not already canceled)
                $cancelBtn = '';
            ...
            ->addColumn('review', function($query) {
                if ($query->order_status === 'delivered') {
                    return "<button class='btn btn-primary getReviewBtn' data-order-id='" . $query->id . "'> Review</button>";
                }
            })
            ->rawColumns(['order_status', 'action', 'payment_status','review'])
            ->setRowId('id');
    }
